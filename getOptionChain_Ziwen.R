...
getOptionChainZiwen <- function(Symbols, Exp=NULL, src="yahoo", ...) {
...
getOptionChain.yahoo <- function(Symbols, Exp, ...)
{
  ...
  NewToOld <- function(x) {
    ...
    d <- with(x, data.frame(Strike=strike, LTD=lasttradedate, Last=lastprice, Chg=change,
  ...
  dftables$calls$LTD <- anytime(dftables$calls$LTD)
  dftables$puts$LTD <- anytime(dftables$puts$LTD)
  dftables
}
